            ...
            <div v-show="is_login">
              <div class="form-group">
                ...
                <input class="form-input"
                       :class="{'is-error': errors.has('login.name')}"
                       v-validate="'required|min:3|max:20'"
                       data-vv-as="名字" name="name" v-model="loginData.name"
                       type="text" placeholder="名字"/>
                <p class="form-input-hint is-success" v-show="errors.has('login.name')">
                  {{ errors.first('login.name')}}</p>
              </div>
              <!--密码-->
              <div class="form-group">
                <input class="form-input"
                       :class="{'is-error': errors.has('login.password')}"
                       v-validate="'required|min:6|max:16'"
                       data-vv-as="密码" name="password" v-model="loginData.password"
                       type="password" placeholder="密码"/>
                <p class="form-input-hint is-success" v-show="errors.has('login.password')">
                  {{ errors.first('login.password')}}</p>
              </div>
              <div class="form-group">
          ...
          <form data-vv-scope="register" @submit.prevent>
            <div v-if="!is_login">
              ...
              <div class="form-group">
                <!--name input-->
                <input class="form-input"
                       :class="{'is-error': errors.has('register.name')}"
                       v-validate="'required|min:3|max:20'"
                       data-vv-as="名字" name="name" v-model="registerData.name"
                       type="text" placeholder="你的 名字"/>
                <p class="form-input-hint is-success" v-show="errors.has('register.name')">
                  {{ errors.first('register.name')}}</p>
              </div>
              <!--密码-->
              <div class="form-group">
                <input class="form-input"
                       :class="{'is-error': errors.has('register.password')}"
                       v-validate="'required|min:6|max:16'"
                       data-vv-as="密码" name="password" v-model="registerData.password"
                       type="password" placeholder="你的 密码"/>
                <p class="form-input-hint is-success" v-show="errors.has('register.password')">
                  {{ errors.first('register.password')}}</p>
              </div>
              <!--注册码-->
              <div class="form-group">
                <input class="form-input"
                       :class="{'is-error': errors.has('register.code')}"
                       v-validate="'required'"
                       data-vv-as="注册码" name="code" v-model="registerData.code"
                       type="text" placeholder="你的 注册码"/>
    ...
    methods: {
      ...
      async login(user_data){
        ...
        if (this.loginData.remember){
          localStorage.rememberData = JSON.stringify(this.loginData)
        }
        //更新用户信息
