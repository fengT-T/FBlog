        ...
        <ul class="menu">
          <li class="menu-item">
            <div class="tile tile-centered">
              <div class="tile-icon">
                <img v-lazy="userInfo.headImgUrl+'?imageView2/1/w/200/h/200/interlace/1/q/75|imageslim'" class="avatar"> </div>
              <div class="tile-content"> {{userInfo.name}} </div>
            </div>
          ...
          <li class="menu-item">
            <a href="javascript:void(0);" @click="select = 'Create'" :class="{'active':select === 'Create'}"> Create </a>
          </li>
          <li class="menu-item">
      ...
      <div class="column col-8">
        ...
        <div v-show="select === 'Settings'">
          <div class="form-group">
            ...
            <input class="form-input" type="text" placeholder="New Name" :class="{'is-error': errors.has('name')}" v-validate="'min:3|max:20'" data-vv-as="名字" name="name" v-model="modifyData.name" />
            <p class="form-input-hint is-success" v-show="errors.has('name')"> {{ errors.first('name')}} </p>
          </div>
          <div class="form-group">
            <label class="form-label">Password</label>
            <input class="form-input" type="password" placeholder="New Password" :class="{'is-error': errors.has('password')}" v-validate="'required|min:6|max:16'" data-vv-as="密码" name="password" v-model="modifyData.password" />
            <p class="form-input-hint is-success" v-show="errors.has('password')"> {{ errors.first('password')}} </p>
          </div>
          <div class="form-group">
            <label class="form-label">Description</label>
            <input class="form-input" type="text" :class="{'is-error': errors.has('desc')}" v-validate="'required|max:100'" data-vv-as="描述" name="desc" placeholder="New Description" v-model="modifyData.desc" />
            <p class="form-input-hint is-success" v-show="errors.has('desc')"> {{ errors.first('desc')}} </p>
          </div>
          <div class="form-group">
            <label class="form-label">headImgUrl</label>
            <input class="form-input" type="text" :class="{'is-error': errors.has('headImgUrl')}" v-validate="'required'" data-vv-as="描述" name="headImgUrl" placeholder="headImgUrl" v-model="modifyData.headImgUrl" />
            <p class="form-input-hint is-success" v-show="errors.has('headImgUrl')"> {{ errors.first('headImgUrl')}} </p>
          </div>
        ...
        <div v-show="select === 'ImageUpload'">
          <div class="columns uploadImg">
            <div class="column col-3" v-for="(item,index) in imageList" :key="index">
        ...
        <div v-show="select === 'Create'">
          <div class="form-group">
            <input class="form-input" v-model="edit.data.title" type="text" placeholder="Title" /> </div>
          <div class="form-group">
          ...
          <div class="popover popover-top form-group">
            ...
            <div class="popover-container">
              <div class="card">
                ...
                <div class="card-body">
                  <ol>
                    <li>这个编辑器可以随意输入HTML,包括危险的包含xss的JavaScript的代码， 之所以允许这样做是因为这是一个web技术博客，需要在页面执行JavaScript </li>
                    <li>由于这个编辑器的存在，这个博客仅仅适用于个人和你信任（不喜欢捣乱的）的小伙伴</li>
                    <li>最后也是最重要的一点，你可以拖动（多个）图片到浏览器上传到7牛云</li>
                  </ol>
                </div>
                <div class="card-footer"> By Feng </div>
              </div>
            </div>
          </div>
...
export default {
  ...
  methods: {
    ...
    async userInfoModify () {
      ...
      try {
        let userData = (await this.$http.post('/user/modify', this.modifyData)).data
        this.$store.dispatch('incrementToastAndShift', {
          desc: '修改成功',
          'toast-success': true
        })
        this.$store.commit('setUserInfo', userData)
        window.localStorage.removeItem('rememberData')    // 清除登录数据算了，反正我怠惰
        this.modifyData = {}
      } catch (e) {
