...
<template>
  <div>
        ...
        <button class="btn btn-link">
          <div class="dropdown">
            <a class="btn btn-link dropdown-toggle" tabindex="0">
              <i class="icon icon-bookmark"></i> {{label}}
              <i class="icon icon-caret"></i>
            </a>
            <ul class="menu">
              <li class="menu-item" v-for="(item,index) in labelList" :key="index">
                <a :href="'#/list/'+item" @click="label = item"> {{item}} </a>
              </li>
            </ul>
          </div>
        </button>
    ...
    <!--登录注册控件-->
    <login-register v-show="show_login" @close="show_login=false"></login-register>
  </div>
</template>
<script>
